...
train_data = pd.read_csv(r'D:\download\train.csv')
test_data = pd.read_csv(r'D:\download\test.csv')
...
score = cross_val_score(XGB, train_data, train_data_target, cv=8).mean()
print("在训练集上的平均5折交叉验证分数:", score)

# 通过上面构造好的模型,对训练数据集进行评估
model = XGB.fit(x_train, y_train)
y_pred = model.predict(x_test)
print("验证数据的准确率: ", accuracy_score(y_test, y_pred))#所有的预测正确（正类负类）的占总的比重
print("验证数据的精确率: ", precision_score(y_test, y_pred))#真正正确的占所有预测为正的比例
print("验证数据的召回率: ", recall_score(y_test, y_pred))#真正正确的占所有实际为正的比例
print("验证数据的Macro-F1: ", f1_score(y_test, y_pred, average="macro"))#F1值为算数平均数除以几何平均数，且越大越好，将Precision和Recall的上述公式带入会发现，当F1值小时，True Positive相对增加，而false相对减少，即Precision和Recall都相对增加，即F1对Precision和Recall都进行了加权。

# 使用上述模型,对测试数据进行预测
test_pred = model.predict(test_data)
test_pred = pd.DataFrame(test_pred, columns=["bad_good"])
sub = pd.concat([test_CUST_ID, test_pred], axis=1)
sub.to_csv(r'C:\Users\86188\Desktop\submission1.csv')